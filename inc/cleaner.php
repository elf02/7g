...
function remove_jpeg_compression()
{
    return 100;
}
...
function remove_core_block_styles()
{
...
function attachment_redirect_not_found()
{
    if (is_attachment()) {
        global $wp_query;
        $wp_query->set_404();
...
function disable_attachment_canonical_redirect_url(string $url)
{
...
function disable_attachment_link(string $url, int $id)
{
...
function disable_attachment_slug_reservation(string $slug, string $id, string $status, string $type)
{
    if ($type !== 'attachment') {
        return $slug;
    }
